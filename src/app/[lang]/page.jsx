...
export default async function Home(props) {
  ...
  const trustPoints = [
    dict.home?.stats?.threatDetection || '99.9% Threat Detection',
    dict.home?.stats?.securitySystem || '7-Layer Security System',
    dict.home?.stats?.realtimeMonitoring || '24/7 Real-time Monitoring'
  ];
  ...
  return (
    <div className="page-shell page-home">
        ...
        <div className="container">
          <div className="hero-split">
            <div className="hero-content hero-content-left">
              <span className="section-kicker">Enterprise Payment Security Platform</span>
              ...
              <p className="hero-subtitle">
                {dict.home?.heroSubtitle || 'PayShield combines advanced cryptography with proactive detection so your team can operate confidently at scale.'}
              </p>
              ...
              <div className="trust-chips" role="list" aria-label="Trust highlights">
                {trustPoints.map((item) => (
                  <span key={item} className="trust-chip" role="listitem">{item}</span>
                ))}
              </div>
            </div>

            <aside className="hero-panel" aria-label="Security performance snapshot">
              <h3>Live Protection Snapshot</h3>
              <div className="panel-metric-grid">
                ...
                <div className="panel-metric-card">
                  <strong>8/8</strong>
                  <span>Security Layers Active</span>
                </div>
              </div>
            </aside>
          </div>
      ...
      <section className="features-section">
        <div className="container">
          <div className="section-header">
            <span className="section-kicker">Defense Architecture</span>
          ...
          <div className="features-grid features-grid-pro">
            ...
            <FeatureCard
              ...
              description={featureCards.behaviorCamera?.description || 'Behavioral analytics that monitors interaction patterns continuously'}
              features={[
                ...
                featureCards.behaviorCamera?.feature3 || 'Anomaly detection models'
              ]}
            />
            <FeatureCard
              ...
              features={[
                ...
                featureCards.alertSystem?.feature3 || 'Escalation-ready operations workflow'
              ]}
            />
          </div>
      ...
      <section className="how-it-works">
        <div className="container">
          <div className="section-header">
            <span className="section-kicker">Operational Flow</span>
            <h2 className="section-title">{dict.home?.howItWorksTitle || 'How PayShield Protects You'}</h2>
          </div>
          <div className="steps-grid steps-grid-pro">
            <div className="step">
            ...
            <div className="step">
              ...
              <p>{steps.step2Desc || 'Credentials are protected with deterministic controls and risk-aware checks.'}</p>
            </div>
            <div className="step">
              <div className="step-number">3</div>
              <h3>{steps.step3Title || 'Continuous Monitoring'}</h3>
              <p>{steps.step3Desc || 'Every session is evaluated across device, behavior, and transaction signals.'}</p>
            </div>
            <div className="step">
              <div className="step-number">4</div>
              <h3>{steps.step4Title || 'Instant Response'}</h3>
              <p>{steps.step4Desc || 'Threats are isolated, alerts are routed, and normal users remain uninterrupted.'}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="cta-section">
        <div className="container">
          <div className="cta-box cta-box-pro">
            <h2>{dict.home?.ctaReady || 'Ready to Experience Unbreakable Security?'}</h2>
            <p>{dict.home?.ctaSubtext || 'Join teams that trust PayShield for secure payment operations.'}</p>
