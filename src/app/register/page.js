    ...
    try {
      ...
      if (data.success) {
        ...
        if (otpType === 'email') {
          ...
          // Phone verified, close modal
          setShowOTPModal(false);
          
          if (data.isFullyVerified && data.token) {
            ...
            alert('Verification complete! Please login.');
    ...
    try {
      ...
      const response = await fetch('/api/users/register', {
        ...
        body: JSON.stringify({
          ...
          deviceDna: deviceId,
          ipAddress: ipAddress,
          locationCountry: location ? `${location.lat},${location.lng}` : 'unknown',
          locationCity: '',
      ...
      if (response.ok && data.user) {
        ...
        setOtpType('email');
        setOtpIdentifier(data.user.email);
        setShowOTPModal(true);
        await sendOTP(data.user.email, 'email');
        setLoading(false);
      } else {
      ...
      <div className="auth-container">
        <div className="auth-box register-box">
          ...
          <form onSubmit={handleSubmit} className="auth-form">
            ...
            >
              {loading ? 'Creating Account...' : (step === 1 ? 'Continue to Security Questions' : 'Create Account')}
            </button>
          </form>
